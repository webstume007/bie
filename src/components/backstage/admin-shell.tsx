...
import { LayoutDashboard, Building2, Users, FileText, Settings, LogOut, Menu } from 'lucide-react';
...
export function AdminShell({ children }: { children: React.ReactNode }) {
  ...
  const navItems = [
    ...
    { name: t.settings, href: '/backstage/settings', icon: Settings },
  ];
    ...
    <div className="min-h-screen bg-background flex flex-col">
      ...
      `}>
        ...
        <nav className="flex-1 overflow-y-auto py-4 px-3 space-y-1">
          {navItems.map((item) => {
            ...
            const isReallyActive = item.href === '/backstage' ? pathname === '/backstage' : isActive;
      ...
      <header className="h-16 bg-slate-950 dark:bg-slate-900 text-white flex items-center justify-between px-4 sm:px-6 lg:px-8 shadow-md z-30 sticky top-0">
        <div className="flex items-center gap-4">
          ...
          <Link href="/backstage" className="flex items-center gap-3 rtl:mr-2 rtl:lg:mr-0 hover:opacity-80 transition-opacity">
            <img 
              src="/logo.webp" 
              alt="BIE" 
