...
export function TeacherShell({ children }: { children: React.ReactNode }) {
  ...
  const t = {
    ...
    ur: {
      ...
      logout: 'لاگ آؤٹ',
      teacherPanel: 'ٹیچر پورٹل',
    }
  }[language];

  const navItems = [
    { name: t.dashboard, href: '/teacher', icon: LayoutDashboard },
    { name: t.classes, href: '/teacher/classes', icon: Users },
    { name: t.marks, href: '/teacher/marks', icon: GraduationCap },
  ];
    ...
    <div className="min-h-screen bg-background flex flex-col">
      ...
      `}>
        ...
        <nav className="flex-1 overflow-y-auto py-4 px-3 space-y-1">
          {navItems.map((item) => {
            ...
            const isReallyActive = item.href === '/teacher' ? pathname === '/teacher' : isActive;
      ...
      <header className="h-16 bg-slate-950 dark:bg-slate-900 text-white flex items-center justify-between px-4 sm:px-6 lg:px-8 shadow-md z-30 sticky top-0">
        <div className="flex items-center gap-4">
          ...
          <Link href="/teacher" className="flex items-center gap-3 rtl:mr-2 rtl:lg:mr-0 hover:opacity-80 transition-opacity">
            <img 
              src="/logo.webp" 
              alt="BIE" 
        ...
        <div className="flex items-center gap-3">
          <div className="hidden sm:flex items-center justify-center px-3 py-1 rounded-full bg-blue-500/10 border border-blue-500/20 text-blue-400 text-xs font-bold tracking-wider mr-2">
            TEACHER
          </div>
          <LanguageSwitcher />
          <ThemeToggle />
